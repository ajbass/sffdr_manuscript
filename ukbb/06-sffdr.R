...
pdf <- pdf %>% select(trait, downsample, `#CHROM`, OBS_CT, POS, ID, indep_snps, BETA, SE,  T_STAT, P, type)
cdf <- cdf %>% select(trait, downsample, `#CHROM`, OBS_CT, POS, ID, indep_snps, BETA, SE,  T_STAT,P, type)
...
for (i in 1:10) {
  ...
  meta <- sffdr:::gwasQvalue(tmp_cdf$P, indep_snps = indep_snps_inform, pi0.method = "bootstrap")
  marg <- sffdr:::gwasQvalue(p, indep_snps = indep_snps_inform, pi0.method = "bootstrap")
