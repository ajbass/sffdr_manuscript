...
fcdf <- fcdf %>% select(trait, downsample, `#CHROM`, OBS_CT, POS, ID, BETA, SE,  T_STAT,P, type)
...
for (trait in primary) {
  # Merge data sets
  out_EGPA <- read.delim(paste0("../../analysis/egpa_analysis/processed/", trait, ".tsv"), sep = "\t")
  out_EGPA <- out_EGPA %>%
    left_join(df_LD) %>%
    left_join(map)
  ...
  ret_df <-  data.frame(id = 1:length(p),
                     p = p,
                     fp  = fp_i,
