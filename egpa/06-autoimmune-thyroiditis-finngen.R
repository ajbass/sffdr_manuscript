...
# Autoimmune thyroiditis from finngen
finn_r12 <- read_tsv("./finngen_R12_E4_THYROIDITAUTOIM.gz", col_select = c(1:7, 9:13)) 
colnames(finn_r12) <- c("chr", "bpos", "a2", "a1", "snpid", "nearest_gene", "pval", "beta", "se", "freq", "af_alt_cases", "af_alt_controls")
finn_r12 <- finn_r12 %>% 
...
ra <- read_tsv("./RA_Okada_24390342_1-hg38.tsv.gz", col_select = c(1:7, 7, 11))#12
ra <- ra %>%
  rename(snpid = SNPID,
         chr = CHR38,
         bpos = BP38, 
...
hypothyroid <- read_tsv("./hypothyroid-gcst90013893-37.tsv", col_select = c(2:6,  11,15)) #7
...
saveRDS(out_train, "./06-ath-r12.rds")
